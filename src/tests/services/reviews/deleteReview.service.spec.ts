...
describe("Create an review", () => {
  ...
  test("Should delete one review", async () => {
    ...
    const deleteReview = await deleteReviewService(review.id);

    const oneReview = await listUserReviewsService(user_id);

    const oneReviewExist = oneReview?.reviews.find(
      (element) => element.id === review.id
    );

    expect(oneReviewExist).toEqual(undefined);
  });
});
